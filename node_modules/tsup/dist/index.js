...
var _chunkJEJ5VAZVjs = require('./chunk-JEJ5VAZV.js');
...
var _chunkPPWAWKPLjs = require('./chunk-PPWAWKPL.js');
...
var generateExternal = async (external) => {
  ...
  for (const item of external) {
    ...
    const deps = await _chunkPPWAWKPLjs.getProductionDeps.call(void 0, pkgPath);
    result.push(...deps);
  }
  return result;
};
async function runEsbuild(options, {
  ...
  const pkg = await _chunkPPWAWKPLjs.loadPkg.call(void 0, process.cwd());
  const deps = await _chunkPPWAWKPLjs.getProductionDeps.call(void 0, process.cwd());
  const external = [
  ...
  if (result && result.warnings && !_chunkPPWAWKPLjs.getSilent.call(void 0, )) {
...
var sizeReporter = () => {
  return {
    ...
    buildEnd({ writtenFiles }) {
      _chunkPPWAWKPLjs.reportSize.call(void 0, 
        this.logger,
        this.format,
...
var import_rollup_plugin_hashbang = _chunkB7UJSB4Njs.__toESM.call(void 0, _chunkPPWAWKPLjs.require_dist.call(void 0, ));
...
var normalizeOptions = async (logger, optionsFromConfigFile, optionsOverride) => {
  ...
  _chunkPPWAWKPLjs.setSilent.call(void 0, options.silent);
...
async function build(_options) {
  const config = _options.config === false ? {} : await _chunkPPWAWKPLjs.loadTsupConfig.call(void 0, 
    process.cwd(),
  ...
  await Promise.all(
    [...Array.isArray(configData) ? configData : [configData]].map(
      async (item) => {
        const logger = _chunkPPWAWKPLjs.createLogger.call(void 0, item == null ? void 0 : item.name);
        const options = await normalizeOptions(logger, item, _options);
        logger.info("CLI", `tsup v${_chunkJEJ5VAZVjs.version}`);
        if (config.path) {
        ...
        const mainTasks = async () => {
          ...
          if (!((_a = options.dts) == null ? void 0 : _a.only)) {
            ...
            let depsHash = await _chunkPPWAWKPLjs.getAllDepsHash.call(void 0, process.cwd());
              ...
              watcher.on("all", async (type, file) => {
                ...
                if (options.watch === true) {
                  if (file === "package.json" && !buildDependencies.has(file)) {
                    const currentHash = await _chunkPPWAWKPLjs.getAllDepsHash.call(void 0, process.cwd());
